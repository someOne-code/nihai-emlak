...
const ADMIN_CATALOG_TIMEOUT_MESSAGE = "Admin katalog isteği zaman aşımına uğradı.";
...
async function requestAdminCatalogJson<T = unknown>(
  ...
  const response = await requestAdminCatalogResponse(url, init, options);
...
async function requestAdminCatalogResponse(
  url: string,
  init: RequestInit,
  options: AdminCatalogClientOptions,
): Promise<Response> {
  ...
  try {
    ...
    if (isAbortError(err)) {
      throw new AdminCatalogClientError(ADMIN_CATALOG_TIMEOUT_MESSAGE, 408);
    }
    throw err;
  } finally {
    abort.cleanup();
...
function createRequestAbort(options: AdminCatalogClientOptions): {
  signal: AbortSignal;
  cleanup: () => void;
} {
