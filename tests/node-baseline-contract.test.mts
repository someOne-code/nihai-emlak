...
test("node baseline script remains wired through npm test", async () => {
  ...
  assert.equal(
    ...
    "npm run test:payment-callback-security && npm run test:security-hardening && npm run test:phase8-admin-listings && npm run test:admin-dashboard && npm run test:admin-users && npm run test:phase9a-content && npm run test:phase9b-catalog && npm run test:admin-operations && npm run test:admin-communications && npm run test:admin-system && npm run test:sale-leads && npm run test:public-site && npm run test:trendyol-market-intel && npm run test:admin-backoffice-e2e:runner && npm run typecheck && npm run lint",
  );
  assert.equal(
  ...
  assert.equal(
    packageJson.scripts?.["test:trendyol-market-intel"],
    "node --experimental-strip-types --test tests/trendyol-market-intel-demo-fallback.test.mts",
  );
  assert.match(
