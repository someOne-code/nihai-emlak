-- Filter admin reservations RPC to only return reservations that have entered
-- the checkout pipeline (i.e. have at least one order).
-- Pre-checkout "ghost" reservations without any order record are NOT operational
-- and must not consume the admin's pagination limit.

-- Overwrite the 4-arg overload that does the actual query work.
...
begin
  ...
  from (
    ...
    where (p_status is null or r.status = p_status)
      -- Only return reservations that have entered the checkout pipeline
      and exists (
